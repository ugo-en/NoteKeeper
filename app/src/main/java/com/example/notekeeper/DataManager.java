...
public class DataManager {
    ...
    public NotebookInfo getNotebook(String id) {
        for (NotebookInfo course : mNoteBooks) {
    ...
    public List<NoteInfo> getNotes(NotebookInfo course) {
        ArrayList<NoteInfo> notes = new ArrayList<>();
        for(NoteInfo note:mNotes) {
            if(course.equals(note.getNotebook()))
                notes.add(note);
        }
        return notes;
    }

    public int getNoteCount(NotebookInfo course) {
        int count = 0;
        for(NoteInfo note:mNotes) {
            if(course.equals(note.getNotebook()))
                count++;
    ...
    private void initializeCourses() {
        mNoteBooks.add(initializeCourse1());
        mNoteBooks.add(initializeCourse2());
        mNoteBooks.add(initializeCourse3());
        mNoteBooks.add(initializeCourse4());
    }

    public void initializeExampleNotes() {
        final DataManager dm = getInstance();

        NotebookInfo course = dm.getNotebook("android_intents");
        ...
        course = dm.getNotebook("android_async");
        ...
        course = dm.getNotebook("java_lang");
        ...
        course = dm.getNotebook("java_core");
    ...
    private NotebookInfo initializeCourse1() {
        ...
        return new NotebookInfo("android_intents", "Android Programming with Intents", modules);
    }

    private NotebookInfo initializeCourse2() {
        List<ModuleInfo> modules = new ArrayList<>();
        ...
        return new NotebookInfo("android_async", "Android Async Programming and Services", modules);
    }

    private NotebookInfo initializeCourse3() {
        List<ModuleInfo> modules = new ArrayList<>();
        ...
        return new NotebookInfo("java_lang", "Java Fundamentals: The Java Language", modules);
    }

    private NotebookInfo initializeCourse4() {
        List<ModuleInfo> modules = new ArrayList<>();
        ...
        return new NotebookInfo("java_core", "Java Fundamentals: The Core Platform", modules);
    }
    //endregion
